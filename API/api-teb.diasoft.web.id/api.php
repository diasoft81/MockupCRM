...
namespace Tqdev\PhpCrudApi\Database {
    ...
    class GenericDB
    {
        ...
        public function createSingle(ReflectedTable $table, array $columnValues) /*: ?String*/
        {
            ...
            switch ($this->driver) {
				case 'mysql':
					// Ambil GUID berdasarkan urutan terbaru CreatedDate
					$stmt = $this->query(
						'SELECT `' . $pkName . '` FROM `' . $tableRealName . '` ORDER BY `CreatedDate` DESC LIMIT 1',
						[]
					);
					break;

				case 'sqlite':
					// SQLite sintaks yang serupa untuk query terbaru
					$stmt = $this->query(
						'SELECT `' . $pkName . '` FROM `' . $tableRealName . '` ORDER BY `CreatedDate` DESC LIMIT 1',
						[]
					);
				...
				default:
					throw new \Exception('Unsupported database driver for auto-generated GUID');
    }
			
			// Kembalikan primary key langsung karena GUID disediakan atau di-generate
			//return $columnValues[$pkName];
	
            $pkValue = $stmt->fetchColumn(0);
			// Jika tipe Primary Key adalah GUID (CHAR(36)), langsung return nilainya
			if ($table->getPk()->getType() == 'char' && $table->getPk()->getLength() == 36) {
				return $pkValue;
			}
